...
<body>
              ...
              <table class="table table-hover">
                ...
                <tbody>
                  <tr>
                    ...
                    <td><fmt:formatDate value="${rplandetail.requestdate}"/> ~ <fmt:formatDate value="${rplandetail.approvaldate}"/></td>
                  </tr>
  ...
  <input type="hidden" name="rplanno" value="${param.rplanno }"/>
      <div class="modal-header">
        ...
        <h4 class="modal-title" id="exampleModalLongTitle" >검토계획 수정# ${param.rplanno }</h4>
      ...
      function updateProc(){
    		if(confirm("수정하시겠습니까?")){
    			...
    			//$("#frm02").attr("action","${path}/updateBoard.do");
    			$("#frm02").submit();
    		}
    	}
      var proc ="${proc}"
    		if(proc=="upt"){
    			if(!confirm("수정성공\n조회리스트화면으로 이동하시겠습니까?")){
    				location.href="${path}/ReviewPlanDetail.do?rplanno="+$("#frm02 [name=rplanno]").val();
    			}else{
    				location.href="${path}/rplanlist.do";
    			}
    		}

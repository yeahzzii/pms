...
<!-- **********************************************************************************************************************************************************
   ...
   if(sessionVal == ""){
				...
				<div class="row">
					<div class="col-md-12">
						<div class="text-right mb">
							<input type="button" onclick="goInsert('${param.rplanno}')" value="등록" class="btn btn-theme">
						</div>
					</div>
					<div class="col-md-12">
							...
							<table class="table table-hover text-center">
								...
								<h4>검토계획명</h4>
									<tr>
										...
										<th class="text-center">위험종류</th>
										<th class="text-center">위험도</th>
										<th class="text-center">등록자</th>
										<th class="text-center">등록일</th>
										<th class="text-center">수정/삭제</th>
										<th class="text-center">시정 조치</th>
									</tr>
								</thead>
								<tbody>
									<c:forEach var="risk" items="${risk}" varStatus="status">
										<tr><td>${status.count}</td><td>${risk.riskContent}</td>
											<td>${risk.riskTypeName}</td>
											<td>${risk.riskDeg}</td><td>${risk.registrant}</td>
											<td><fmt:formatDate value="${risk.regDate}"/></td>
											<td>
												<button class="btn btn-primary btn-xs" onclick="goUpdate('${risk.riskNo}')">
													<i class="fa fa-pencil"></i></button>
												<button class="btn btn-danger btn-xs" onclick="deleteRisk('${risk.riskNo}', '${risk.rplanno}')">
													<i class="fa fa-trash-o"></i></button>
											</td>
											<td>
												<button class="btn btn-info btn-xs" onclick="goFbplanlist('${risk.riskNo}')">
	...
	<script type="text/javascript">
		function goInsert(rplanno) {
			location.href="${path}/riskInsertForm.do?rplanno="+rplanno;
		}
		function deleteRisk(riskNo, rplanno) {
			if (confirm("삭제하시겠습니까?")) {
				location.href="${path}/deleteRisk.do?riskNo="+riskNo
				alert("삭제하였습니다.")
				location.href="${path}/risk.do?rplanno="+rplanno
			}
		}
		function goUpdate(riskNo) {
			if (confirm("수정하시겠습니까?")) {
				location.href="${path}/riskUpdateForm.do?riskNo="+riskNo;
			}
		}
		function goFbplanlist(riskNo) {
			location.href="${path}/fbplanlist.do?riskNo="+riskNo+"&prjNo="+'${param.prjNo}';
		}
	</script>
</body>
